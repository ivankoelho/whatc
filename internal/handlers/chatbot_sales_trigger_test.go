...
// TestExecChatButtons_CreateOpportunityStillFiresWhenTeamIDInvalid verifies
// that create_opportunity: true still opens a sales funnel entry even when
// the same button's team_id fails to resolve (foreign-org id here, mirroring
// TestExecChatButtons_UnknownTeamIDLeavesContactTeamNil) — the two fields are
// independent, and the team_id early-exit must not skip create_opportunity.
func TestExecChatButtons_CreateOpportunityStillFiresWhenTeamIDInvalid(t *testing.T) {
	app, org, account, contact, session := newGraphTestFixtures(t)
	...
	flow := &models.ChatbotFlow{
		BaseModel: models.BaseModel{ID: uuid.New()}, OrganizationID: org.ID,
		WhatsAppAccount: account.Name, Name: "menu-opportunity-bad-team", IsEnabled: true,
		Graph: models.JSONB{
			"version": 2, "entry_node": "b1",
			"nodes": []any{
				map[string]any{"id": "b1", "type": "buttons", "label": "menu",
					"config": map[string]any{"body": "O que deseja?",
						"buttons": []any{
							map[string]any{"id": "realizar_pedido", "title": "Realizar pedido",
								"team_id": foreignTeam.ID.String(), "create_opportunity": true},
						}}},
				map[string]any{"id": "e1", "type": "end", "label": "done"},
			},
			"edges": []any{
	...
	require.NoError(t, app.runChatGraph(account, contact, session, flow, "start", "", nil))           // park at b1
	require.NoError(t, app.runChatGraph(account, contact, session, flow, "", "realizar_pedido", nil)) // pick flagged button with bad team_id
